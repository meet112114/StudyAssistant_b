...
const getResources = async (req, res) => {
    ...
    try {
        ...
        const resourcesWithFlags = resources.map(r => {
...
const getPublicSummary = async (req, res) => {
    try {
        ...
        let summaryItem = await Summary.findOne({ resource: id });
        if (!summaryItem) {
            return res.status(403).json({ message: "Summary not generated by admin yet. Please ask the admin to generate it." });
        }
        res.json(summaryItem);
    } catch (err) {
...
const generateSummary = async (req, res) => {
    try {
        ...
        // Find resource (admins can access any resource; non-admins can only access their own)
        const query = { _id: id };
        ...
        // If summary already exists, do not regenerate
        let summaryItem = await Summary.findOne({ resource: id });
        if (summaryItem) {
            return res.status(400).json({ message: "Summary has already been generated for this resource.", summary: summaryItem });
        }
        ...
        res.status(200).json({ message: "Summary generated successfully", summary: summaryItem });
    } catch (err) {
